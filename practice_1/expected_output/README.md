...
- [x] **Documentation (`docs/interface.md`)**
  - [x] 5-8 host questions with required data listed
  - [x] All data sources listed (git log, file history, etc.)
  - [x] Mapping table: 3+ Resources, 2+ Tools, 1+ Prompt
- [x] **Resources (server code)**
  - [x] 3 Resources with URIs like `git-activity://summary/{repo_path}`
  - [x] Resources return static data (not empty)
- [x] **Tools (server code)**
  - [x] `analyze_hotspots` schema defined and implemented
  - [x] `analyze_commit_patterns` schema defined and implemented
- [x] **Prompt Template (server code)**
  - [x] 1 workflow prompt referencing your tools (e.g. "Repo health review")
- [x] **Security**
  - [x] `config/allowed_repos.json` exists with >= 1 repo path
  - [x] `repo_path` validation blocks paths outside allowed dirs
  - [x] `file_path` blocks `../` traversal
  - [x] SSE auth: "TODO: add JWT" comment in server.py
- [x] **Tests pass**
  - [x] `pytest` runs green
